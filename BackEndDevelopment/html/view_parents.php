...
$search_query = $search ? "WHERE first_name LIKE '%$search%' OR last_name LIKE '%$search%' OR pupil_name LIKE '%$search%'" : '';
$limit = 6; 
$page = isset($_GET['page']) ? $_GET['page'] : 1;
$start = ($page - 1) * $limit;
$count_sql = "SELECT COUNT(*) FROM parents $search_query";
$count_result = $conn->query($count_sql);
$total_rows = $count_result->fetch_row()[0];
$total_pages = ceil($total_rows / $limit);
$sql = "SELECT parent_id_pk, first_name, last_name, date_of_birth, address, phone_number, email, relationship_to_pupil, pupil_name 
        FROM parents 
        $search_query 
        LIMIT $start, $limit";
...
<head>
    ...
    <title>View All Parents/Guardians</title>
...
<!-- Main Content -->
    <div class="display_parents">
        <div class="container">
            <h1>All Parent/Guardian Records</h1>
            <div class="search-bar">
                <form method="get" action="view_parents.php">
                    <input type="text" id="searchInput" name="search" placeholder="Search by name or pupil..." value="<?php echo $search; ?>">
            ...
            <div class="table-container">
                ...
                if ($result->num_rows > 0) {
                    ...
                    echo "<thead><tr><th>ID</th><th>First Name</th><th>Last Name</th><th>Date of Birth</th><th>Address</th><th>Phone Number</th><th>Email</th><th>Relationship to Pupil</th><th>Pupil Name</th><th>Edit</th><th>Delete</th></tr></thead>";
                    echo "<tbody>";
                    while ($row = $result->fetch_assoc()) {
                        echo "<tr>";
                        echo "<td>" . $row["parent_id_pk"] . "</td>";
                        ...
                        echo "<td>" . $row["relationship_to_pupil"] . "</td>";
                        echo "<td>" . $row["pupil_name"] . "</td>";
                        echo "<td><a href='edit_parent.php?id=" . $row["parent_id_pk"] . "' class='edit-btn'>Edit</a></td>";
                        echo "<td><button type='button' class='delete-btn' onclick='confirmDelete(" . $row["parent_id_pk"] . ")'>Delete</button></td>";
                        echo "</tr>";
                    }
                    echo "</tbody></table>";
                } else {
                    echo "<p>No parents/guardians found.🙂</p>";
                }
                ?>
            </div>
            <div class="pagination">
                ...
                for ($i = 1; $i <= $total_pages; $i++) {
                    echo "<a href='view_parents.php?page=$i&search=$search'";
